...
const CostEfficiencyScore: React.FC<{ percentage: number }> = ({
  percentage,
}) => {
...
export default function HeroStats({ stats }: HeroStatsProps) {
  const savingsPercentage = calculateSavingsPercentage(
    stats.couldHaveSpent,
    stats.actuallySpent
  );
  const savingsAmount = stats.couldHaveSpent - stats.actuallySpent;

  return (
    <div className="card bg-gradient-to-br from-indigo-50 to-purple-50 p-2 border-0 shadow-lg">
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-1">
            Without Autopilot optimization
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
          ...
          <p className="text-xs text-gray-500 mt-2">
            Efficiency score based on cost reduction
          </p>
        </div>
      </div>
    </div>
